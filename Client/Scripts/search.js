...
//grabbing the elements by id on the content page
const searchInput = document.getElementById('search') //in search bar
...
searchInput.addEventListener("input", x => {
    ...
    Array.from(content).forEach(item => { 
        ...
        item.style.display = title.includes(value) ? '' : 'none'; //if the title is included keep it visible other wise set to none and "hide it"
    })
})
